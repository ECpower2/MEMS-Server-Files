...
beam_x = zeros(1,100*m);  beam_w = beam_x;
for i = 1:m
    beam_x(100*(i-1)+1:100*i) = linspace(x(2*i-1),x(2*i+1));
end
beam_w = a(4*i-3)*beam_x.^3/3 + a(4*i-2)*beam_x.^2/2 + ...
    a(4*i-1)*beam_x + a(4*i);
plot(beam_x, beam_w)
xlabel('Position')
ylabel('Deflection')
