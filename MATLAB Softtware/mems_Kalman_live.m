function outfile = mems_Kalman_live(filename,nnodes,nsamples,nref)
...
if nargin < 2
    nnodes = input('How many nodes are connected? ');
end

% get number of samples to collect if not specified
if nargin < 3
    nref = input('How many samples should be taken? (rate = 100Hz): ');
end
...
data = readXmn(nnodes,nsamples);
% data = readXmnUSB(nnodes,nsamples);

% determine orientation of reference node
for j = 1:nnodes
    % identify node
    nodeID = data(1,10*j-9);
    
    if nodeID == nref
        % create post-process file
        csv_ref = [filename '-ref.csv'];
        fid = fopen(csv_ref,'w');
...
for j = 1:nnodes
    ...
    if nnodes == 1
        ...
        outfile = [filename '-node' num2str(nodeID) '.csv'];
    end
    fid = fopen(outfile,'w');
    header = ['time,EqKm_x,EqKm_y,EqKm_z,Sens_x,Sens_y,Sens_z,'...
        'a_x,a_y,a_z,m_x,m_y,m_z,g_x,g_y,g_z,'...
    ...
    c0 = 10*j-8;
    [time,qSens,acc,mag,gyro,wKalman,qKalman,J] = KF_node(nsamples,c0,data);   
...
% read data
acc = data(:,c0:c0+2) - accBias;                         % mn
mag = data(:,c0+3:c0+5) - magBias;                         % mn
gyro = [data(:,c0+8) -data(:,c0+6) -data(:,c0+7)]*pi/180; % ned
