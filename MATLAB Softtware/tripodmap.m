function data = tripodmap(filename,newfile)
% Allows user to select data and assign to target angle
...
while more=='y'
    ...
    n = n2-n1+1;
    newdata = zeros(n,25);
    angle = input('Enter target angle in degrees: ');
    for i = 1:n
        j = n1+i-1;
        newdata(i,:) = [angle time(j) Gqw(j) Gqx(j) Gqy(j) Gqz(j) Lqw(j)...
            Lqx(j) Lqy(j) Lqz(j) rx(j) ry(j) rz(j) lx(j) ly(j) lz(j)...
            ax(j) ay(j) az(j) mx(j) my(j) mz(j) gx(j) gy(j) gz(j)];
    end
