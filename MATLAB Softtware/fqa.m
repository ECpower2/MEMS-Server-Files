...
mag_field = [19.1083 .2264 45.1032];  % Kluyverweg, Delft, July 2015
n = norm(mag_field);
n_x = mag_field(1)/n;
n_y = mag_field(2)/n;
...
%     earth_m = vq(q_prod(q_e,q_r),body_m);
    earth_m = vq(q_quot([1 0 0 0],q_prod(q_r,q_e)),body_m);
    ...
    if i==1; q_a0 = q_a; end
    q_an = q_quot(q_a,q_a0);
        
    % calculate orientation quaternion
    q(i,:) = q_prod(q_an,q_prod(q_e,q_r));
