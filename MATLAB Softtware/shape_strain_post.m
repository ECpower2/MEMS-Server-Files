function shape_strain_post(model,filename)
% reads MotionNode exported stream (filename), calculates shape and
% interfaces with COMSOL model (model)

[p1,p2,p3,~] = beam_coeffs(filename);
...
function [p1,p2,p3,rsq] = beam_coeffs(filename)
...
function outfile = mems_Kalman_postprocess(filename,nref)
...
for j = 1:n_nodes
    ...
    EqKalman = q2eulerN(q_quotN(qKalman,qKal_ref));                 % ned
