function outfile = mems_Kalman_post_ref(filename,nnodes,nref)
...
refname = [filename '-node' num2str(nref) '.csv'];
dref = dlmread(refname,',',1,0);
...
for j = 1:nnodes
    ...
    outfile = [filename '-node' num2str(nodeID-1) '-post-ref' ...
        num2str(nref) '.csv'];
    fid = fopen(outfile,'w');
    ...
    [~,qSens_ref,~,~,~,~,qKal_ref,~] = KF_node(nsamples,dref);
